...
def pc_game(play_state):
    ...
    while run:
        ...
        if state == 0:
            screen.blit(p2_text, (P2.x - (player_size // 3), P2.y - P2_size // 2 - text_distance))
            #screen.blit(p1_text, (P1.x - (P1_size / 5), P1.y - P1_size))
            screen.blit(p1_text, (P1.x - (player_size // 3), P1.y - P1_size // 2 - text_distance))
        else:
            screen.blit(p1_text, (P1.x - (player_size // 3), P1.y - P1_size // 2 - text_distance))
            screen.blit(p2_text, (P2.x - (player_size // 3), P2.y - P2_size // 2 - text_distance))
        #--------------------------------
        ...
        #------繪製倒數-------
        if cd >= 0:
            count = count_down(cd, color, volume, count) + 0.2
            if count >= fps:
                cd -= 1
                count = 0
            continue
